...
public class MainFrame extends JFrame {
    ...
    private void sendMessage() {
        ...
        // Process message in a background thread using SwingWorker
        SwingWorker<String, Void> worker = new SwingWorker<String, Void>() {
            ...
            protected void done() {
                ...
                try {
                    ...
                    // Add AI response to the chat panel
                    chatPanel.addMessage(response, false);
                } catch (Exception ex) {
                    ex.printStackTrace();
                    // Show an error message to the user on the EDT
                    JOptionPane.showMessageDialog(MainFrame.this, "Error: " + ex.getMessage(), "API Error", JOptionPane.ERROR_MESSAGE);
                } finally {
                    // Re-enable the send button on the EDT
